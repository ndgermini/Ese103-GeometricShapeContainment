package ese103;

import geometricshapes.*;

public class Ese103 
{
	public static void main(String[] args) 
	{
		Circle c[] = new Circle[3];
		c[0] = new Circle(10, 20, 30);
		c[1] = new Circle(10, 20, 60);
		c[2] = new Circle(20, 40, 80);
		Triangle t[] = new Triangle[3];
		t[0] = new Triangle(10, 20, 30);
		t[1] = new Triangle(20, 40, 60);
		t[2] = new Triangle(20, 40, 80);
		
		controlContainment(c[0], c[1]);
		controlContainment(c[0], c[2]);
		controlContainment(c[1], c[0]);
		controlContainment(c[1], c[2]);
		controlContainment(c[2], c[0]);
		controlContainment(c[2], c[1]);
		System.out.println();
		
		controlContainment(t[0], t[1]);
		controlContainment(t[0], t[2]);
		controlContainment(t[1], t[0]);
		controlContainment(t[1], t[2]);
		controlContainment(t[2], t[0]);
		controlContainment(t[2], t[1]);
		System.out.println();
		
		controlContainment(c[0], t[1]);
		controlContainment(c[0], t[2]);
		controlContainment(c[1], t[0]);
		controlContainment(c[1], t[2]);
		controlContainment(c[2], t[0]);
		controlContainment(c[2], t[1]);
		System.out.println();
		
		controlContainment(t[0], c[1]);
		controlContainment(t[0], c[2]);
		controlContainment(t[1], c[0]);
		controlContainment(t[1], c[2]);
		controlContainment(t[2], c[0]);
		controlContainment(t[2], c[1]);
		System.out.println();
	}
	
	private static void controlContainment(Circle c0, Circle c1)
	{
		if(c0.contains(c1))
			System.out.println(c0 + " contiene il -------> " + c1);
		else
			System.out.println(c0 + " non contiene il ---> " + c1);
	}
	
	private static void controlContainment(Triangle t0, Triangle t1)
	{
		if(t0.contains(t1))
			System.out.println(t0 + " contiene il -------> " + t1);
		else
			System.out.println(t0 + " non contiene il ---> " + t1);
	}
	
	private static void controlContainment(Circle c0, Triangle t1)
	{
		if(c0.contains(t1))
			System.out.println(c0 + " contiene il -------> " + t1);
		else
			System.out.println(c0 + " non contiene il ---> " + t1);
	}
	
	private static void controlContainment(Triangle t0, Circle c1)
	{
		if(t0.contains(c1))
			System.out.println(t0 + " contiene il -------> " + c1);
		else
			System.out.println(t0 + " non contiene il ---> " + c1);
	}
	
	
}
	
		
